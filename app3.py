...
def main():
    ...
    if choise == menu[0]:
        ...
        if st.checkbox('전부 선택하기'):
            pred_choise = st.multiselect('입력받을 데이터를 정해주세요.', df.columns, max_selections=len(df.columns) - 1, default=df.columns.values[:-1])
        else:
        ...
        if pred_choise:
            ...
            y_choise = st.selectbox('예측할 데이터를 선택해주세요.', set(df.columns) - set(pred_choise))
            ...
            if st.button('예측결과 보기'):
                ...
                elif y_choise in int_list: # 시간은 올림해서 받기.
                    import math
                    ans, acc = reg(onehot_df, X, new_data, y_choise)
...
def reg(onehot_df, X, new_data, y_choise):
    y = onehot_df[y_choise] # none타입으로 뜸. 이거 수정하기.
